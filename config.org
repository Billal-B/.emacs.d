...
*** quick move [[https://emacs.stackexchange.com/questions/5603/how-to-quickly-copy-move-file-in-emacs-dired][see]]
#+begin_src emacs-lisp
...
(when (featurep! 'ivy)
  ...
  (general-define-key
   ...
   "C-N" 'counsel-projectile-find-file)
...
(when (featurep! 'helm)
  ...
  (general-define-key
   ...
   "RET" 'bb/helm-ag-mode-jump-other-window)
  )
#+end_src
*** lisp
#+begin_src emacs-lisp
(require 'highlight)
(defun bb/helm-ag-mode-jump-other-window ()
  "Not documented."
  (interactive)
...
** Neotree
#+begin_src emacs-lisp
(defun bb/neotree-hook ()
  (visual-line-mode -1))
(use-package neotree
  :hook (neotree-mode . bb/neotree-hook)
  :config
  (require 'neotree)
...
(use-package symbol-overlay
  ...
  :hook ((prog-mode . symbol-overlay-mode)
	 ...
	 (lsp-mode . (lambda () (symbol-overlay-mode -1))) ;; redundent with lsp
...
(use-package avy
  ...
  (setq avy-keys '(?h ?j ?k ?l ?m ?g ?y ?u ?i ?o ?p ?t ?f ?v ?b ?n ?a ?z ?e ?r ?q ?s ?d ?w ?x))
  )
...
(defhydra hydra-avy (:exit t :hint nil)
 ...
 Line^^       Region^^        Goto
----------------------------------------------------------
 [_y_] yank   [_Y_] yank      [_s_] timed char  [_c_] char
 [_m_] move   [_M_] move      [_z_] word        [_Z_] any word
...
(general-define-key "M-z" 'hydra-avy/body)
#+end_src
...
(use-package lsp-ui
  ...
  (setq lsp-modeline-code-actions-enable t) ;; see https://github.com/emacs-lsp/lsp-mode/pull/1781
...
** Scala
Taken [[https://scalameta.org/metals/docs/editors/emacs.html][here]]
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
(when (featurep! 'ivy)
   (mode-leader emacs-lisp-mode-map
...
(defun bb/terminal-here ()
  (interactive)
  (let ((workdir (if (projectile-project-root)
                     (projectile-project-root)
                   default-directory)))
    (call-process-shell-command
     (concat "xfce4-terminal --working-directory " workdir) nil 0)))
#+end_src
...
** Modeline
Warning : don't use feebleline as it's broken with other packages (for ex visual-replace)
#+BEGIN_SRC emacs-lisp
(setq bb/modeline 'doom)
